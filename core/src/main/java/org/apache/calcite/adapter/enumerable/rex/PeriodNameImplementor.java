...
class PeriodNameImplementor extends MethodNameImplementor {
  ...
  private Expression getExpression(RexToLixTranslator translator,
      Expression operand, BuiltInMethod builtInMethod) {
    final Expression unboxedOperand =
        RexImpTable.NullAs.NOT_POSSIBLE.handle(operand);
    ...
    return Expressions.call(builtInMethod.method.getDeclaringClass(),
        builtInMethod.method.getName(), unboxedOperand, locale);
  }
}

// End PeriodNameImplementor.java
